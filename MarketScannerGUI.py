...
def displayChart(ticker, name, marketCap, folderName, timeframe, period, filterUpBars, filterDownBars, consec,
    ...
    # Add titles
    titleString = 'Ticker: ' + ticker + '; Company Name: ' + \
    ...
    if (isConsec):
        if (aboveEMA == priceAboveEMA and belowEMA == priceBelowEMA):
            fig.write_image(folderName + "/" + ticker +
                            ".jpeg", engine="kaleido")
            wrote = True
    return wrote
...
def scan(minMarketCap, minVolume, sector, filterUpBars, filterDownBars, consec, aboveEMA, belowEMA, timeframe, period):
    print("Scanning for stocks with min market cap", numerize.numerize(
        minMarketCap), ", minVolume", numerize.numerize(minVolume), ".")
    ...
    tickersFrame = pd.DataFrame(
        tickers, columns=['Symbol', 'Name', 'Market Cap', 'Sector', 'Volume'])
    ...
    for ticker in tqdm(tickersList):
        try:
            name = tickersFrame[tickersFrame['Symbol']
                                == ticker]['Name'].values[0]
            marketCap = int(
                tickersFrame[tickersFrame['Symbol'] == ticker]['Market Cap'].values[0])
